...
const ChatBubble: React.FC<ChatBubbleProps> = ({
  ...
  return (
    <div className={`fixed z-50 ${positionClasses[position]}`}>
      <button
        onClick={onClick}
        className={`relative w-16 h-16 rounded-full border-2 border-white shadow-lg transition-all duration-300 transform hover:scale-110 focus:outline-none focus:ring-4 focus:ring-blue-500/30 bg-white hover:bg-gray-50 ${
...
export const ChatIntegration: React.FC = () => {
  ...
  const [notificationCount, setNotificationCount] = useState(2); // Notification state

  const handleChatToggle = () => {
    setIsOpen(!isOpen);
    // When the chat is opened, clear notifications
    if (!isOpen) {
      setNotificationCount(0);
    }
    console.log('Portal chat toggled:', !isOpen);
  };
  ...
  return (
    <ChatBubble
      ...
      position="bottom-right"
      isActive={isOpen}
      hasNotification={notificationCount > 0} // Now depends on state
      notificationCount={notificationCount} // Now depends on state
